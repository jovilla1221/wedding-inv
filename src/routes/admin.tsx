...
function AdminPage() {
  ...
  const handleAddGuest = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("guests").insert([
      { name: newName.trim(), slug: uniqueSlug }
    ]);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">Kelola daftar tamu dan buat link unik untuk dibagikan.</p>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-4 mt-2 text-sm text-slate-600">
                <span className="bg-green-50 text-green-700 px-2 py-1 rounded-md border border-green-200">
                  Hadir: {rsvps.filter(r => r.is_attending).reduce((acc, curr) => acc + curr.guest_count, 0)} Orang
                </span>
                ...
                <tbody className="divide-y divide-slate-100">
                  {rsvps.map((rsvp) => (
                    <tr key={rsvp.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4 text-slate-600 text-sm italic max-w-[200px] truncate" title={rsvp.message}>
                        {rsvp.message || "-"}
                      </td>
                    </tr>
                  ))}
